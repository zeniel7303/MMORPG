...
void Dummy::RandomTask()
{
	...
	itoa(dummyNum, chattingPacket->id, 10);
	strcpy(chattingPacket->chattingBuffer, "Test");
...
void Dummy::FSM()
{
	...
	if (heartBeatTime >= 2.0f)
	{
		Packet* CheckAlivePacket =
			reinterpret_cast<Packet*>(m_loginDummy->GetSendBuffer()->
				GetBuffer(sizeof(Packet)));
		CheckAlivePacket->Init(SendCommand::C2Zone_CHECK_ALIVE, sizeof(Packet));

		m_loginDummy->GetSendBuffer()->Write(CheckAlivePacket->size);
		m_loginDummy->Send(reinterpret_cast<char*>(CheckAlivePacket), CheckAlivePacket->size);
