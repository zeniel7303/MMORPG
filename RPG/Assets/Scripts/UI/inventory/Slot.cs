...
public class Slot : MonoBehaviour
{
    ...
    public Stack<Item> _slot;       // 슬롯을 스택으로 만든다.
    public Text _text;       // 아이템에 개수를 표현해줄 텍스트.
    public Sprite _defaultImg; // 슬롯에 있는 아이템을 다 사용할 경우 아무것도 없는 이미지를 넣어줄 필요가 있다.

    private Image ItemImg;
    private bool isSlot;     // 현재 슬롯이 비어있는지?

    public Item ItemReturn() { return _slot.Peek(); } // 슬롯에 존재하는 아이템이 뭔지 반환.
    public bool ItemMax(Item item) { return ItemReturn()._maxCount > _slot.Count; } // 겹칠수 있는 한계치를 넘으면.   
    ...
    void Start()
    {
        ...
        float Size = _text.gameObject.transform.parent.GetComponent<RectTransform>().sizeDelta.x;
        _text.fontSize = (int)(Size * 0.5f);
    ...
    public void AddItem(Item item)
    {
        // 스택에 아이템 추가.
        _slot.Push(item);
        UpdateInfo(true, item._defaultImg);
    }
    ...
    public void UpdateInfo(bool isSlot, Sprite sprite)
    {
        ...
        if (_slot.Count > 1)
            _text.text = _slot.Count.ToString();
        else
            _text.text = "";
    }
}
