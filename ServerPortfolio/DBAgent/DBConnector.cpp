...
void DBConnector::Register(RegisterPacket_DBAgent* _packet)
{
	...
	if ((mysql_row = mysql_fetch_row(m_result)) == NULL)
	{
		...
		MainThread::getSingleton()->AddToSendQueue(RegisterSuccessPacket);
	}
	//해당 id가 존재함 - 회원가입 불가능
	else
	{
...
void DBConnector::GetUserInfo(RequireUserInfoPacket_DBAgent* _packet)
{
	...
	if ((mysql_row = mysql_fetch_row(m_result)) == NULL)
	{
		...
		MainThread::getSingleton()->AddToSendQueue(GetSessionInfoFailedPacket);
	}
	else
	{
		//성공
		...
		MainThread::getSingleton()->AddToSendQueue(sessionInfoPacket);
	}
}
...
void DBConnector::UpdateUser(UpdateUserPacket* _packet)
{
	...
	if ((mysql_row = mysql_fetch_row(m_result)) == NULL)
	{
		...
		MainThread::getSingleton()->AddToSendQueue(updateUserFailedPacket);
	}
	else
	{
		//성공
		...
		MainThread::getSingleton()->AddToSendQueue(updateUserSuccessPacket);
	}
}
...
void DBConnector::GetMonsterInfo()
{
	...
	MainThread::getSingleton()->AddToSendQueue(monstersInfoPacket);
}
#endif
