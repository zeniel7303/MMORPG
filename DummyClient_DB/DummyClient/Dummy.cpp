...
{
	RegisterPacket_DBAgent* packet =
		reinterpret_cast<RegisterPacket_DBAgent*>(GetSendBuffer()->
			GetBuffer(sizeof(RegisterPacket_DBAgent)));;
	strcpy(packet->id, "zeniel");
	strcpy(packet->password, "1029");
	packet->socket = m_socket;

	packet->Init(SendCommand::TEST, sizeof(RegisterPacket_DBAgent));
